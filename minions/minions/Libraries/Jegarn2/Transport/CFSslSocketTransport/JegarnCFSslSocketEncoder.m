...
- (void)stream:(NSStream*)sender handleEvent:(NSStreamEvent)eventCode {
    ...
    if (eventCode & NSStreamEventHasSpaceAvailable) {
        DDLogVerbose(@"[JegarnCFSslSocketEncoder] NSStreamEventHasSpaceAvailable");
        if (![self applySSLSecurityPolicy:sender withEvent:eventCode]) {
            ...
            DDLogVerbose(@"[JegarnCFSslSocketEncoder] NSStreamEventHasSpaceAvailable error %@", self.error);
        } else {
